...
public class Resize2 extends JPanel implements ComponentListener {
	...
	public int setFirstHeigth(int listLength) {// 초기 패널 높이 설정
		...
		return rst;
	} // setFirstHeigth method end

	@Override
	public void componentResized(ComponentEvent e) {
		jfrGrid.removeAll();// 그리드 형식이 바뀔 때 마다 새로 그린다.
		int column = (int) (mc.getMf().getWidth() - 120) / 165;// 컬럼의 개수 결정, 버튼1개의 width() 165
		if (column < 3)
			column = 3;// 최소컬럼수 =3
		int row = (int) (listLength / column);// row의 수를 결정
		if (listLength % column != 0)
			row++; // 행의 개수 결정
		if (row < 3)
			row = 3;// 최소 행수 =3
		...
		int borderWidth = (int) ((mc.getMf().getWidth() - 540 - borderWidthSet) / 2);
